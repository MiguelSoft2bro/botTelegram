...
class TestStartHandler:
    ...
    @pytest.mark.asyncio
    async def test_start_shows_help_when_disconnected(self, tmp_path):
        ...
        assert "/connect" in reply_text

    @pytest.mark.asyncio
    async def test_start_shows_status_when_connected(self, tmp_path):
        b = load_bridge(tmp_path)
        self._bootstrap(b, tmp_path)
        b.connect_session(chat_id=100, user_id=429591886, opencode_session_id="test-sess")
        ...
        assert "test-sess" in reply_text

    @pytest.mark.asyncio
    async def test_unauthorized_user_rejected(self, tmp_path):
        b = load_bridge(tmp_path)
        self._bootstrap(b, tmp_path)
...
class TestDisconnectHandler:
    ...
    async def test_disconnect_clears_session(self, tmp_path):
        b = load_bridge(tmp_path)
        b.atomic_write(b.SESSION_PATH, dict(b.EMPTY_SESSION))
        b.connect_session(chat_id=100, user_id=429591886, opencode_session_id="active-sess")

        update = make_update(user_id=429591886, chat_id=100)
        ctx = make_context()
        await b.disconnect_handler(update, ctx)
...
class TestSessionsHandler:
    ...
    async def test_sessions_lists_available(self, tmp_path):
        ...
        # Mock the list_opencode_sessions function
        with patch.object(b, "list_opencode_sessions", return_value=(True, "session-1\nsession-2")):
    ...
    async def test_sessions_handles_error(self, tmp_path):
        ...
        with patch.object(b, "list_opencode_sessions", return_value=(False, "command not found")):
            await b.sessions_handler(update, ctx)

        calls = update.message.reply_text.call_args_list
        assert "Error" in calls[1][0][0]
...
class TestMessageHandler:
    def _connected_session(self, b, chat_id: int = 100, user_id: int = 429591886, session_id: str = "test-sess"):
        b.atomic_write(b.STATE_PATH, dict(b.EMPTY_STATE))
        b.connect_session(chat_id=chat_id, user_id=user_id, opencode_session_id=session_id)
    ...
    async def test_message_calls_opencode_run(self, tmp_path):
        ...
        with patch.object(b, "run_opencode", return_value=(True, "Hello! How can I help?")) as mock_run:
            ...
            mock_run.assert_awaited_once_with("test-sess", "Hello Claude")
...
class TestRunOpencode:
    ...
    async def test_run_opencode_success(self, tmp_path):
        ...
        with patch("asyncio.create_subprocess_exec", return_value=mock_process) as mock_exec:
            success, output = await b.run_opencode("session-123", "Hello")
    ...
    async def test_run_opencode_failure(self, tmp_path):
        ...
        with patch("asyncio.create_subprocess_exec", return_value=mock_process):
            success, output = await b.run_opencode("bad-session", "Hello")
    ...
    async def test_run_opencode_timeout(self, tmp_path):
        ...
        with patch("asyncio.create_subprocess_exec", return_value=mock_process):
            success, output = await b.run_opencode("session-123", "Long running task")
